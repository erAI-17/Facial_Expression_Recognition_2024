...
class LocalGlobalFusionNet(nn.Module):
   ...
   def __init__(self, rgb_model, depth_model):
      ...
      for stage in self.stages.keys():
            ...
            self.num_patches[stage] = (self.stages[stage][1] // self.patch_size)**2 * self.stages[stage][0]
            self.proj_layers[stage] = nn.Linear(self.num_patches[stage], 768)
      ...
      self.seq_len = (self.patch_size ** 2) * len(self.stages.keys()) #concatenate early, mid and late features
...
#only late, no local-global attention, no projection layer
class TRIALLocalGlobalFusionNet(nn.Module):
   """Visual Transformer Feature Fusion with Attention Selective fusion.
      Combines multiple Transformer encoder layers for classification
   """
   def __init__(self, rgb_model, depth_model):
      super(TRIALLocalGlobalFusionNet, self).__init__()      
      num_classes, valid_labels = utils.utils.get_domains_and_labels(args)
      ...
      self.num_layers = 4
      self.mlp_dim = 3072 #?MLP dimension INTERNAL to each transformer layer (std is 2048)
      self.seq_len = 49 #concatenate early, mid and late features
